...
class ReplayingHammerer {
 private:
  ...
  // New: Multi-Thread - Add a mutex to guard mem access
  mutable std::mutex mem_mtx;
 ...
 private:
  ...
  // New: Multi-Thread - Change the static to atomic
  static std::atomic<double> last_reproducibility_score;
  static std::vector<std::string> collect_json_files(const std::string& json_path); // New: To get json files from a path
 ...
 public:
  ...
  size_t replay_patterns_multi_threaded(const std::string& json_filename,
                                       const std::unordered_set<std::string> &pattern_ids,
                                       size_t sweep_bytes_per_thread, int num_threads);
  ...
  // New: Multi-Thread - Returns a thread-local parameter set instead of mutating this->params
  FuzzingParameterSet derive_FuzzingParameterSet_values_ret(HammeringPattern &pattern,
                                                            PatternAddressMapper &mapper);

  // New: Multi-Thread - Sweep using a provided, thread-local FuzzingParameterSet and an explicit base address
  SweepSummary sweep_pattern(FuzzingParameterSet &fp, HammeringPattern &pattern,
