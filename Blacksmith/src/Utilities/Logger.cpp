// A MODIFIED VERSION by NIMA SAYADI
...
// New: Multi-Thread
std::mutex Logger::io_mtx;

void Logger::initialize() {
  std::lock_guard<std::mutex> lk(io_mtx);
  instance.logfile = std::ofstream();
  instance.bitflip_logfile = std::ofstream();

  const char* root_env = std::getenv("ROOT_PATH");
  std::string root_path = (root_env != nullptr) ? root_env : ".";
  
  std::string logfile_filename = root_path + "/logs/stdout.log";
  std::string BitFlipLogfile_filename = root_path + "/logs/bitflips.log";

  std::cout << "Writing into logfiles " FF_BOLD << logfile_filename << F_RESET << std::endl;
  ...
  instance.bitflip_logfile.open(BitFlipLogfile_filename, std::ios::out | std::ios::app);
  instance.timestamp_start = (unsigned long) time(nullptr);
}

void Logger::mirror_to_terminal(const std::string &message, bool newline) {
  std::cout << message;
  if (newline) std::cout << "\n";
}
...
void Logger::close() {
  ...
  instance.bitflip_logfile << "\n======================\n" << "Total BitFlips: " << instance.bitflip_count << " Bit Flips\n";
  instance.bitflip_logfile.close();
}

void Logger::log_info(const std::string &message, bool newline) {
  std::lock_guard<std::mutex> lk(io_mtx);
  std::string formatted = std::string(FC_CYAN) + "[+] " + message + F_RESET;
  instance.logfile << "[+] " << message;
  if (newline) instance.logfile << "\n";
  mirror_to_terminal(formatted, newline);
}

void Logger::log_highlight(const std::string &message, bool newline) {
  std::lock_guard<std::mutex> lk(io_mtx);
  std::string formatted = FC_MAGENTA + std::string(FF_BOLD) + "[+] " + message + F_RESET;
  instance.logfile << "[+] " << message;
  if (newline) instance.logfile << "\n";
  mirror_to_terminal(formatted, newline);
}
...
void Logger::log_error(const std::string &message, bool newline) {
  std::lock_guard<std::mutex> lk(io_mtx);
  std::string formatted = std::string(FC_RED) + "[-] " + message + F_RESET;
  instance.logfile << "[-] " << message;
  if (newline) instance.logfile << "\n";
  mirror_to_terminal(formatted, newline);
}
...
void Logger::log_data(const std::string &message, bool newline) {
  ...
  instance.logfile << message;
  if (newline) instance.logfile << "\n";
  mirror_to_terminal(message, newline);
}
...
void Logger::log_bitflip(volatile char *flipped_address, uint64_t row_no, unsigned char actual_value,
  ...
  ss  << "[!] Flip " << std::hex << (void *) flipped_address << ", "
      << std::dec << "row " << row_no << ", "
  ...
  std::string plain_text = ss.str();
  std::string colored_text = std::string(FC_GREEN) + plain_text + std::string(F_RESET);
  
  instance.logfile << plain_text;
  instance.bitflip_logfile << plain_text;
  if (newline) {
    instance.logfile << "\n";
    instance.bitflip_logfile << "\n";
  }
  mirror_to_terminal(colored_text,newline);

  instance.bitflip_count++; // Just count bit flips
}

void Logger::log_success(const std::string &message, bool newline) {
  std::lock_guard<std::mutex> lk(io_mtx);
  std::string formatted = std::string(FC_GREEN) + "[!] " + message + F_RESET;
  instance.logfile << "[!] " << message;
  if (newline) instance.logfile << "\n";
  mirror_to_terminal(formatted,newline);
}
...
void Logger::log_failure(const std::string &message, bool newline) {
  std::lock_guard<std::mutex> lk(io_mtx);
  std::string formatted = std::string(FC_RED_BRIGHT) + "[-] " + message + F_RESET;
  instance.logfile << "[-] " << message;
  if (newline) instance.logfile << "\n";
  mirror_to_terminal(formatted,newline);
}
