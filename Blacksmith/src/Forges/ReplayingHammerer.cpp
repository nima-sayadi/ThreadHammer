...
std::atomic<double> ReplayingHammerer::last_reproducibility_score{0.0};
...
size_t ReplayingHammerer::hammer_pattern(FuzzingParameterSet &fuzz_params, CodeJitter &code_jitter,
  ...
  for (; num_tries <= num_reps; num_tries++) {
    ...
    if (check_flips_after_each_rep || num_tries==num_reps - 1) {
      ...
      // New: Multi-Thread - Guard mem.check_memory(...) and mem.flipped_bits
      size_t num_bitflips = 0;
      {
        std::lock_guard<std::mutex> lk(mem_mtx);
        num_bitflips = mem.check_memory(mapper, true, verbose_memcheck);
...
// New: Multi-Thread - New helper that returns a thread-local FuzzingParameterSet
FuzzingParameterSet ReplayingHammerer::derive_FuzzingParameterSet_values_ret(HammeringPattern &pattern,
                                                                             PatternAddressMapper &mapper) {
  CodeJitter &jitter = mapper.get_code_jitter();

  FuzzingParameterSet fp(static_cast<int>(pattern.total_activations / pattern.num_refresh_intervals));
  fp.set_total_acts_pattern(pattern.total_activations);
  fp.set_hammering_total_num_activations(jitter.total_activations);

  int agg_intra_dist = 0, agg_inter_dist = 0;
  bool use_seq_addrs = false;
  mapper.compute_mapping_stats(pattern.agg_access_patterns, agg_intra_dist, agg_inter_dist, use_seq_addrs);

  fp.set_use_sequential_aggressors(Range<int>(use_seq_addrs, use_seq_addrs));
  fp.set_agg_inter_distance(agg_inter_dist);
  fp.set_agg_intra_distance(agg_intra_dist);

  return fp;
}

// New: Multi-Thread - New sweep overloads that use the provided fp and an explicit base address
struct SweepSummary ReplayingHammerer::sweep_pattern(FuzzingParameterSet &fp, HammeringPattern &pattern,
                                                     PatternAddressMapper &mapper, size_t num_reps,
                                                     void* sweep_base_addr, size_t size_bytes) {
  return sweep_pattern(fp, pattern, mapper, num_reps, sweep_base_addr, size_bytes, {});
}
...
struct SweepSummary ReplayingHammerer::sweep_pattern(FuzzingParameterSet &fp, HammeringPattern &pattern,
  ...
  auto base_ptr   = reinterpret_cast<uint8_t*>(sweep_base_addr);
  auto max_addr_u = reinterpret_cast<uint64_t>(base_ptr) + size_bytes;

  auto row_start = DRAMAddr(static_cast<void*>(base_ptr)).row;
  auto row_end   = DRAMAddr(reinterpret_cast<void*>(max_addr_u)).row;
  auto num_rows  = row_end - row_start;

  Logger::log_info(format_string("Sweeping pattern %s with mapping %s over %zu MB (thread region), rows=%ld, reps=%zu.",
  ...
  for (long r = 1; r <= num_rows; ++r) {
    ...
    auto num_flips = hammer_pattern(fp, jitter, pattern, mapper, jitter.flushing_strategy,
        jitter.fencing_strategy, num_reps, jitter.num_aggs_for_sync, jitter.total_activations, true,
        jitter.pattern_sync_each_ref, false, false, false, true, true);
...

    {
      std::lock_guard<std::mutex> lk(mem_mtx);
      bflips.insert(bflips.end(), mem.flipped_bits.begin(), mem.flipped_bits.end());
      for (auto bf : mem.flipped_bits) bitflips_list.push_back(bf);
    }

    std::string flipped_rows;
    {
      std::lock_guard<std::mutex> lk(mem_mtx);
      flipped_rows = mem.get_flipped_rows_text_repr();
    }

    init_ss(ss);
    ss << std::setw(10) << r
      << std::setw(12) << mapper.min_row
      << std::setw(12) << mapper.max_row
      << std::setw(13) << num_flips
      << flipped_rows;
    Logger::log_data(ss.str());
  ...
  Logger::log_info("Summary of sweeping pattern:");
  Logger::log_data(format_string("Total corruptions: %ld", total_bit_flips_sweeping));
  size_t z2o_corruptions = 0, o2z_corruptions = 0;
  for (const auto &bf : bflips) {
    z2o_corruptions += bf.count_z2o_corruptions();
    o2z_corruptions += bf.count_o2z_corruptions();
  }
  ...
  return SweepSummary{ z2o_corruptions, o2z_corruptions, bitflips_list };
}

// New: Multi-Thread - New sweep func that uses thread-local params, offline mapping pick, and disjoint ranges
size_t ReplayingHammerer::replay_patterns_multi_threaded(const std::string& json_path, const std::unordered_set<std::string> &pattern_ids, size_t sweep_bytes_per_thread, int num_threads) {

  auto files = collect_json_files(json_path);
  if (files.empty()) {
    Logger::log_error(format_string("No JSON files found at path: %s", json_path.c_str()));
    exit(EXIT_FAILURE);
  }

  if (files.size() == 1) {
    // ======== SINGLE FILE: all threads on same file ========
    Logger::log_info(format_string("Found 1 JSON file. Spawning %d threads for this file.", num_threads));
    Logger::log_info(format_string("Proceeding with strategy 1..."));
    auto patterns = load_patterns_from_json(files[0], pattern_ids);
    if (patterns.empty()) {
      Logger::log_error("No patterns loaded from JSON file. Cannot perform multi-threaded sweeping.");
      exit(EXIT_FAILURE);
    }
  
    Logger::log_info(format_string("Starting multi-threaded sweeping with %d threads, %zu bytes per thread",
                                   num_threads, sweep_bytes_per_thread));
  
    std::vector<std::thread> threads;
    threads.reserve(num_threads);
  
    std::vector<size_t> thread_bitflips(static_cast<size_t>(num_threads), 0);
    std::vector<std::string> thread_filenames(static_cast<size_t>(num_threads));
  
    // copy patterns per thread
    std::vector<std::vector<HammeringPattern>> thread_patterns(static_cast<size_t>(num_threads), patterns);
  
    auto now_sec = []() -> long long {
    ...
    for (int thread_id = 0; thread_id < num_threads; ++thread_id) {
      threads.emplace_back([this, thread_id, &thread_patterns, &thread_bitflips, &thread_filenames,
                        sweep_bytes_per_thread,
                        now_sec, to_iso]() {
        auto& local_patterns = thread_patterns[thread_id];
        thread_filenames[thread_id] = format_string("sweeping-%zuM-P%d.json", sweep_bytes_per_thread / (1024UL*1024UL), thread_id);
  
        // compute disjoint base address for this thread
        auto base_addr = reinterpret_cast<std::uintptr_t>(mem.get_starting_address())
                         + static_cast<size_t>(thread_id) * sweep_bytes_per_thread;
  
        size_t local_bitflips = 0;
  
        #ifdef ENABLE_JSON
        nlohmann::json thread_runs = nlohmann::json::array();
        const auto thread_tp_start = std::chrono::system_clock::now();
        const auto thread_epoch_start = now_sec();
        #endif
  
        for (auto& pattern : local_patterns) {
          // === pick best mapping OFFLINE (no hammering, no shared params) ===
          PatternAddressMapper* best = pattern.address_mappings.empty()
                                       ? nullptr
                                       : &pattern.address_mappings.front();
          size_t best_flips = 0;
          for (auto& m : pattern.address_mappings) {
            size_t flips = m.count_bitflips();
            if (flips > best_flips) { best_flips = flips; best = &m; }
          }
          if (best == nullptr) continue; // no mappings
  
          PatternAddressMapper thread_mapper = *best;
  
          // thread-local FuzzingParameterSet
          auto fp = derive_FuzzingParameterSet_values_ret(pattern, thread_mapper);
  
          // shift mapping into this thread's region
          auto base_row = DRAMAddr(reinterpret_cast<void*>(base_addr)).row;
          int shift_rows = static_cast<int>(base_row) - static_cast<int>(thread_mapper.min_row);
          thread_mapper.shift_mapping(shift_rows, {}); // shift all aggressors
  
          // effective aggressors (optional)
          std::unordered_set<AggressorAccessPattern> direct_effective_aggs;
          if (pattern.is_location_dependent) {
            find_direct_effective_aggs(pattern, thread_mapper, direct_effective_aggs);
          }
  
          #ifdef ENABLE_JSON
          const auto patt_tp_start = std::chrono::system_clock::now();
          const auto patt_epoch_start = now_sec();
          #endif
  
          // sweep within this thread's region
          auto summary = sweep_pattern(fp, pattern, thread_mapper, 10,
                                       reinterpret_cast<void*>(base_addr),
                                       sweep_bytes_per_thread,
                                       direct_effective_aggs);
          local_bitflips += summary.observed_bitflips.size();
  
          #ifdef ENABLE_JSON
          const auto patt_tp_end = std::chrono::system_clock::now();
          const auto patt_epoch_end = now_sec();
          nlohmann::json entry;
          entry["pattern"] = pattern.instance_id;
          entry["mapping"] = thread_mapper.get_instance_id();
  
          // region this thread scanned
          entry["region"]["start_addr"] = base_addr;
          entry["region"]["size_bytes"] = sweep_bytes_per_thread;
  
          // per-pattern timing
          entry["timing"]["start_epoch"] = patt_epoch_start;
          entry["timing"]["end_epoch"]   = patt_epoch_end;
          entry["timing"]["start_iso"]   = to_iso(patt_tp_start);
          entry["timing"]["end_iso"]     = to_iso(patt_tp_end);
  
          nlohmann::json flips;
          flips["zero_to_one"] = summary.num_flips_z2o;
          flips["one_to_zero"] = summary.num_flips_o2z;
          flips["total"]       = summary.num_flips_z2o + summary.num_flips_o2z;
          flips["details"]     = summary.observed_bitflips; // addr/bitmask/etc.
  
          entry["flips"] = flips;
  
          thread_runs.push_back(entry);
          
          #endif
  
          Logger::log_info(format_string("Thread %d: Swept pattern %s (mapping %s), bitflips %zu",
              thread_id, pattern.instance_id.c_str(), thread_mapper.get_instance_id().c_str(),
              summary.observed_bitflips.size()));
        }
  
        thread_bitflips[thread_id] = local_bitflips;
  
        #ifdef ENABLE_JSON
        const auto thread_tp_end = std::chrono::system_clock::now();
        const auto thread_epoch_end = now_sec();
  
        nlohmann::json thread_meta;
        thread_meta["thread_id"]       = thread_id;
        thread_meta["region_start"]    = base_addr;
        thread_meta["region_size"]     = sweep_bytes_per_thread;
        thread_meta["total_bitflips"]  = local_bitflips;
        thread_meta["memory_config"]   = DRAMAddr::get_memcfg_json();
        thread_meta["dimm_id"]         = program_args.dimm_id;
  
        // thread-level timing
        thread_meta["timing"]["start_epoch"] = thread_epoch_start;
        thread_meta["timing"]["end_epoch"]   = thread_epoch_end;
        thread_meta["timing"]["start_iso"]   = to_iso(thread_tp_start);
        thread_meta["timing"]["end_iso"]     = to_iso(thread_tp_end);
  
        nlohmann::json root;
        root["metadata"] = thread_meta;
        root["sweeps"]   = thread_runs;
  
        std::ofstream thread_file(thread_filenames[thread_id]);
        if (thread_file.is_open()) {
          thread_file << root.dump(2) << std::endl;
          thread_file.close();
          Logger::log_info(format_string("Thread %d: Results saved to %s",
                                        thread_id, thread_filenames[thread_id].c_str()));
        } else {
          Logger::log_error(format_string("Thread %d: Failed to save results to %s",
                                          thread_id, thread_filenames[thread_id].c_str()));
        }
        #endif
    ...
    for (auto flips : thread_bitflips) total_bitflips += flips;
  
    Logger::log_success(format_string("Multi-threaded sweeping completed. Total bitflips: %zu", total_bitflips));
    return total_bitflips;
  }
  else {
    // ======== MULTI FILES: one file per thread (cap threads to file count) ========
    const int threads_used = std::min<int>(num_threads, static_cast<int>(files.size()));
    int sweep_chunk = MAX_SWEEP_SIZE / threads_used;
    sweep_bytes_per_thread = MB(sweep_chunk);
    Logger::log_info(format_string("Found %zu JSON files. Spawning %d thread(s), one per pattern file.", files.size(), threads_used));
    Logger::log_info(format_string("Proceeding with strategy 2 or 3..."));
    Logger::log_info(format_string("Found %zu JSON files. Spawning %d thread(s), one per file.", files.size(), threads_used));
    Logger::log_info(format_string("Using sweep_chunk of %d for each thread|json-file", sweep_chunk));

    std::vector<std::thread> threads;
    threads.reserve(threads_used);
    std::vector<size_t> thread_bitflips(static_cast<size_t>(threads_used), 0);
    ...
    for (int tid = 0; tid < threads_used; ++tid) {
      const std::string file_for_thread = files[static_cast<size_t>(tid)];

      threads.emplace_back([this, tid, file_for_thread, &pattern_ids,
                            sweep_bytes_per_thread, &thread_bitflips,
                            now_sec, to_iso]() {
        auto patterns = load_patterns_from_json(file_for_thread, pattern_ids);
        if (patterns.empty()) {
          Logger::log_error(format_string("Thread %d: No patterns in %s", tid, file_for_thread.c_str()));
          thread_bitflips[tid] = 0;
          return;
        }

        auto base_addr = reinterpret_cast<std::uintptr_t>(mem.get_starting_address())
                        + static_cast<size_t>(tid) * sweep_bytes_per_thread;
        ...
        const auto epoch_start = now_sec();
      #endif
        ...
        for (auto& pattern : patterns) {
          ...
          PatternAddressMapper mapper = *best;
          auto fp = derive_FuzzingParameterSet_values_ret(pattern, mapper);

          auto base_row = DRAMAddr(reinterpret_cast<void*>(base_addr)).row;
          int shift_rows = static_cast<int>(base_row) - static_cast<int>(mapper.min_row);
          mapper.shift_mapping(shift_rows, {});
          ...
          auto summary = sweep_pattern(fp, pattern, mapper, 10,
                                      reinterpret_cast<void*>(base_addr),
                                      sweep_bytes_per_thread,
                                      direct_effective_aggs);
          local_bitflips += summary.observed_bitflips.size();
          ...
          nlohmann::json entry;
          entry["source_file"] = file_for_thread;
          entry["pattern"]     = pattern.instance_id;
          entry["mapping"]     = mapper.get_instance_id();
          entry["region"]["start_addr"] = base_addr;
          entry["region"]["size_bytes"] = sweep_bytes_per_thread;
          ...
          Logger::log_info(format_string(
            "Thread %d: %s | pattern %s (mapping %s) -> bitflips %zu",
            tid, file_for_thread.c_str(),
            pattern.instance_id.c_str(),
            mapper.get_instance_id().c_str(),
            summary.observed_bitflips.size()));
        }

        thread_bitflips[tid] = local_bitflips;

      #ifdef ENABLE_JSON
        const auto tp_end = std::chrono::system_clock::now();
        const auto epoch_end = now_sec();

        nlohmann::json meta;
        meta["thread_id"]      = tid;
        meta["source_file"]    = file_for_thread;
        meta["region_start"]   = base_addr;
        meta["region_size"]    = sweep_bytes_per_thread;
        meta["total_bitflips"] = local_bitflips;
        meta["memory_config"]  = DRAMAddr::get_memcfg_json();
        meta["dimm_id"]        = program_args.dimm_id;
        meta["timing"]["start_epoch"] = epoch_start;
        meta["timing"]["end_epoch"]   = epoch_end;
        meta["timing"]["start_iso"]   = to_iso(tp_start);
        meta["timing"]["end_iso"]     = to_iso(tp_end);

        nlohmann::json root;
        root["metadata"] = meta;
        root["sweeps"]   = runs;

        std::filesystem::path p(file_for_thread);
        std::ostringstream out;
        out << "sweeping-" << p.stem().string()
            << "-" << (sweep_bytes_per_thread / (1024UL*1024UL)) << "M"
            << "-T" << tid << ".json";

        std::ofstream f(out.str());
        if (f.is_open()) {
          f << root.dump(2) << std::endl;
          f.close();
          Logger::log_info(format_string("Thread %d: Results saved to %s", tid, out.str().c_str()));
        } else {
          Logger::log_error(format_string("Thread %d: Failed to save results to %s", tid, out.str().c_str()));
        }
      #endif
      });
    }
    ...
    size_t total = 0;
    for (auto v : thread_bitflips) total += v;

    Logger::log_success(format_string(
      "Multi-file sweeping done. Files: %zu, Threads used: %d, Total bitflips: %zu",
      files.size(), threads_used, total));
    return total;
  }

}

// New: to load json files in one path
std::vector<std::string> ReplayingHammerer::collect_json_files(const std::string& json_path) {
  ...
  if (fs::is_regular_file(p, ec)) {
    if (p.extension() == ".json") files.push_back(p.string());
    return files;
  }
