...
#include <fstream>
#include <memory>
#include <cstdint> // New: makes uint64_t visible on some systems!
...
class Logger {
 private:
  ...
  // Seperate BitFlips Log File For ThredHammer
  std::ofstream bitflip_logfile;
  ...
  size_t bitflip_count = 0;

  
  public:
  
  static void mirror_to_terminal(const std::string &message, bool newline);
